...
// Extend array for seamless loop
...
const BookCategories = () => {
  ...
  useEffect(() => {
    if (currentIndex === promotions.length) {
      setTimeout(() => {
        setTransitionEnabled(true); // Disable transition for instant reset
        setCurrentIndex(0);
        requestAnimationFrame(() => setTransitionEnabled(true)); // Re-enable transition smoothly
      }, 1000); 
    }
  }, [currentIndex]);
